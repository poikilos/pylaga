...
class Swarm(pygame.sprite.Group):
    def __init__(self, world_rect, bullet_odds):
        self.bullet_odds = bullet_odds
    ...
    def shoot(self, image, bullet_spritegroup):
        self.tock = random.randint(0, self.bullet_odds)
        if self.tock < len(self):
            self.sprites()[self.tock].shoot(image, bullet_spritegroup)
...
class Enemy(pygame.sprite.Sprite):
    ...
    def __init__(self, swarm, speed, enemy_image, explosion_images):
        self.swarm = swarm
        self.world_rect = swarm.world_rect.copy()
    ...
    def shoot(self, image, spritegroup):
        ...
        tempb.move_by(self.rect.left + self.rect.width / 2,
                      self.rect.bottom)
